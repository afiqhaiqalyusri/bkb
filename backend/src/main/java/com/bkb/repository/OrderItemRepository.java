...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query("SELECT oi.menuItem.id, oi.menuItem.name, SUM(oi.quantity) as totalQty, SUM(oi.quantity * oi.unitPrice) as totalRevenue, SUM(oi.quantity * oi.unitPrice) - SUM(oi.quantity * COALESCE(oi.unitCost, 0)) as estimatedProfit " +
           "FROM OrderItem oi " +
           "JOIN oi.order o " +
