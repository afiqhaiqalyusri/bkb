...
#  Run this in PowerShell after: gh auth login
#
#  Usage:
#    1. Open PowerShell
#    2. gh auth login   (browser-based login, do this once)
#    3. .\scripts\set-secrets.ps1
# =============================================================
...
Write-Host "Tip: Your private key is usually at ~/.ssh/id_rsa or ~/.ssh/id_ed25519" -ForegroundColor DarkGray
$VPS_KEY_PATH = Read-Host "Path to your VPS private key file (e.g. C:\Users\you\.ssh\id_rsa)"
if (Test-Path $VPS_KEY_PATH) {
    ...
    Write-Host "  [WARN] Key file not found. Paste the private key manually:" -ForegroundColor Yellow
    $VPS_KEY = Read-Host "Paste private key content"
    Set-GhSecret "VPS_SSH_PRIVATE_KEY" $VPS_KEY
}

$VPS_HOST = Read-Host "VPS_HOST (IP address or hostname of your VPS)"
Set-GhSecret "VPS_HOST" $VPS_HOST

$VPS_USER = Read-Host "VPS_USER (SSH username, e.g. ubuntu or afiqhaiqal)"
...
$DB_URL = Read-Host "DATABASE_URL [press Enter for default: jdbc:postgresql://host.docker.internal:5432/bkb]"
if ([string]::IsNullOrWhiteSpace($DB_URL)) { $DB_URL = "jdbc:postgresql://host.docker.internal:5432/bkb" }
...
Write-Host "Generate with: openssl rand -hex 64  (run this in Git Bash or WSL)" -ForegroundColor DarkGray

$JWT_SECRET = Read-Host "JWT_SECRET"
Set-GhSecret "JWT_SECRET" $JWT_SECRET

$JWT_REFRESH = Read-Host "JWT_REFRESH_SECRET (must be different from JWT_SECRET)"
...
Write-Host ""
Write-Host "Next: Trigger the pipeline at:" -ForegroundColor White
